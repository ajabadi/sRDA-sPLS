...
sRDAccp <- function(predictor,
  ...
  if (multiple_LV){
    ...
    result <- get_multiple_LVsCpp(X = predictor,
                               Y = predicted,
...
#  Xcr <- X.mat
  ...
  while(CRT > tolerance && WeContinnue && Nr_iterations < max_iterations) {
    ...
    ETA = eigenMapMatMult(Yc, BETA)
    ETA = colScale(ETA)


    XI = eigenMapMatMult(Xcr, ALPHA)
    XI = colScale(XI)
    ...
    ALPH_0 <- switch(penalization,
                ...
                "none" = {
                  solve(eigenMapMatMult(transpose(Xcr), Xcr)) %*% eigenMapMatMult(
                    transpose(Xcr), ETA)
                  },
    ...
    BETA_0 = eigenMapMatMult(solve(crossprod(XI)), 
                             crossprod(XI, Yc))
    BETA_0 = transpose(BETA_0)

    #BETA
    #         compute the vaule of ETAhat^(1),
    #
    #           ETAhat^(1) = SUM_running to q where k=1 ( BETAhat_k^(1) * y_k)
    ETA = eigenMapMatMult(Yc, BETA_0)
    ETA = colScale(ETA)
  ...
  SOLVE_XIXI = crossprod(XI) %*% transpose(XI)
