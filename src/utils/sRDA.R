...
#  Xcr <- X.mat
  ...
  while(CRT > tolerance && WeContinnue && Nr_iterations < max_iterations) {
    ...
    ALPH_0 <- switch(penalization,
                ...
                "none" = {
                  solve(transpose(Xcr) %*% Xcr) %*% transpose(Xcr) %*% ETA
                  },
    ...
    BETA_0 = solve(transpose(XI)%*%XI) %*% transpose(XI) %*% Yc
    BETA_0 = transpose(as.matrix(BETA_0))
  ...
  SOLVE_XIXI = solve(transpose(XI)%*%XI) %*% transpose(XI)
